...
public:
    ...
    conntrack()
        : _lb(make_lw_shared<load_balancer>())
    {}
...
class posix_ap_server_socket_impl : public server_socket_impl
{
    ...
    struct connection
    {
        ...
        connection(pollable_fd xfd, socket_address xaddr, conntrack::handle cth)
            : fd(std::move(xfd))
            , addr(xaddr)
            , connection_tracking_handle(std::move(cth))
        {}
    };
...
public:
    explicit posix_ap_server_socket_impl(int protocol, socket_address sa, compat::polymorphic_allocator<char>* allocator = memory::malloc_allocator)
        : _protocol(protocol)
        , _sa(sa)
...
class posix_server_socket_impl : public server_socket_impl
{
    socket_address _sa;
    int _protocol;
    pollable_fd _lfd;       // 管理着 socket 文件描述符
...
public:
    ...
    {
        // 返回一个已经就绪的 future. future::_state::_u 设置为 state::result, future::_state::_v 设置为 std::unique_ptr<network_stack>
